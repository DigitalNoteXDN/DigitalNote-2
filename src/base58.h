...
public:
    ...
    CDigitalNoteAddress() {}
    CDigitalNoteAddress(const CTxDestination &dest) { Set(dest); }
    CDigitalNoteAddress(const std::string& strAddress) { SetString(strAddress); }
    CDigitalNoteAddress(const char* pszAddress) { SetString(pszAddress); }
...
public:
    ...
    CDigitalNoteSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CDigitalNoteSecret() {}
};

template<typename K, int Size, CChainParams::Base58Type Type> class CDigitalNoteExtKeyBase : public CBase58Data
{
public:
