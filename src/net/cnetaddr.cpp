...
{
	uint256 hash = Hash_bmw512(&ip[0], &ip[16]);
	uint64_t nRet;
...
template void CNetAddr::Serialize<CDataStream>(CDataStream& s, int nType, int nVersion) const;
template void CNetAddr::Unserialize<CDataStream>(CDataStream& s, int nType, int nVersion);
