...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDigitalNoteAmount"))
        settings.setValue("nAnonymizeDigitalNoteAmount", 1000);
    nAnonymizeDigitalNoteAmount = settings.value("nAnonymizeDigitalNoteAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDigitalNoteAmount"))
        SoftSetArg("-anonymizeDigitalNoteamount", settings.value("nAnonymizeDigitalNoteAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeDigitalNoteAmount:
            nAnonymizeDigitalNoteAmount = value.toInt();
            settings.setValue("nAnonymizeDigitalNoteAmount", nAnonymizeDigitalNoteAmount);
            emit AnonymizeDigitalNoteAmountChanged(nAnonymizeDigitalNoteAmount);
            break;
        default:
            break;
        }
