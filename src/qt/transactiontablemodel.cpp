...
public:
    ...
    void refreshWallet()
    {
        ...
        cachedWallet.clear();
        {
            ...
            for(mapWallet_t::iterator it = wallet->mapWallet.begin(); it != wallet->mapWallet.end(); ++it)
            {
    ...
    TransactionRecord *index(int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            if(lockMain)
            {
                ...
                {
                    mapWallet_t::iterator mi = wallet->mapWallet.find(rec->hash);
...
{
    columns << QString() <<  QString() << tr("Date") << tr("Type") << tr("Address") << DigitalNoteUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
	columns[Amount] = DigitalNoteUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    QString str = DigitalNoteUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit);
    if(showUnconfirmed)
    {
