...
{
    widget->setMaxLength(DigitalNoteAddressValidator::MaxAddressLength);
    widget->setValidator(new DigitalNoteAddressValidator(parent));
...
bool parseDigitalNoteURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("DigitalNote"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DigitalNoteUnits::parse(DigitalNoteUnits::XDN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDigitalNoteURI(QString uri, SendCoinsRecipient *out)
{
    // Convert digitalnote:// to DigitalNote:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("digitalnote://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "DigitalNote:");
    }
    QUrl uriInstance(uri);
    return parseDigitalNoteURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DigitalNote.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DigitalNote.lnk
...
{
    return GetAutostartDir() / "DigitalNote.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a DigitalNote.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DigitalNote\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the DigitalNote app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add DigitalNote app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DigitalNote-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DigitalNote-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DigitalNote-Qt"));
