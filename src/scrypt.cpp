...
#define xor_salsa8_R(a, b) (((a) << (b)) | ((a) >> (32 - (b))))
...
static inline void xor_salsa8(unsigned int B[16], const unsigned int Bx[16])
{
	...
	{
		/* Operate on columns. */
		x04 ^= xor_salsa8_R(x00+x12, 7); x09 ^= xor_salsa8_R(x05+x01, 7);
		x14 ^= xor_salsa8_R(x10+x06, 7); x03 ^= xor_salsa8_R(x15+x11, 7);

		x08 ^= xor_salsa8_R(x04+x00, 9); x13 ^= xor_salsa8_R(x09+x05, 9);
		x02 ^= xor_salsa8_R(x14+x10, 9); x07 ^= xor_salsa8_R(x03+x15, 9);

		x12 ^= xor_salsa8_R(x08+x04,13); x01 ^= xor_salsa8_R(x13+x09,13);
		x06 ^= xor_salsa8_R(x02+x14,13); x11 ^= xor_salsa8_R(x07+x03,13);

		x00 ^= xor_salsa8_R(x12+x08,18); x05 ^= xor_salsa8_R(x01+x13,18);
		x10 ^= xor_salsa8_R(x06+x02,18); x15 ^= xor_salsa8_R(x11+x07,18);

		/* Operate on rows. */
		x01 ^= xor_salsa8_R(x00+x03, 7); x06 ^= xor_salsa8_R(x05+x04, 7);
		x11 ^= xor_salsa8_R(x10+x09, 7); x12 ^= xor_salsa8_R(x15+x14, 7);

		x02 ^= xor_salsa8_R(x01+x00, 9); x07 ^= xor_salsa8_R(x06+x05, 9);
		x08 ^= xor_salsa8_R(x11+x10, 9); x13 ^= xor_salsa8_R(x12+x15, 9);

		x03 ^= xor_salsa8_R(x02+x01,13); x04 ^= xor_salsa8_R(x07+x06,13);
		x09 ^= xor_salsa8_R(x08+x11,13); x14 ^= xor_salsa8_R(x13+x12,13);

		x00 ^= xor_salsa8_R(x03+x02,18); x05 ^= xor_salsa8_R(x04+x07,18);
		x10 ^= xor_salsa8_R(x09+x08,18); x15 ^= xor_salsa8_R(x14+x13,18);
	}
